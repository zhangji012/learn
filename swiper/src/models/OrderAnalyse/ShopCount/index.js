...
import { columns1, columns2, columns3, columns4, bar1Data, bar2Data, bar3Data } from './data'
...
class Sort extends Component {
  ...
  handleChangeDate = (value, unit) => {
    ...
    this.setState({
      rangePicker: rangePicker, startTime: startTime, endTime: endTime
    }, () => {
      this.loadAll()
    })
  }
  ...
  onChartClick = (param, echart) => {
    ...
    console.log(selectedBarNum)
    let colorLists = {
      colorList1,
      colorList2,
    }
    this.setState({
      colorLists, selectedBar, selectedBarNum
    }, () => {
      this._loadBar3()
      this._loadTable1()
      this._loadTable2()
  ...
  _loadBar1 = async (params = {}) => {
    let res = await api.shopCount.bar1({
  ...
  _loadBar2 = async (params = {}) => {
    let res = await api.shopCount.bar2({
      beginTime: params.startTime === undefined ? this.state.startTime : params.startTime,
      endTime: params.endTime === undefined ? this.state.endTime : params.endTime,
    }) || {}
    if (res.yAxis.length > 1) {
  ...
  _loadBar3 = async (params = {}) => {
    let res = await api.shopCount.bar3({
      beginTime: params.startTime === undefined ? this.state.startTime : params.startTime,
      endTime: params.endTime === undefined ? this.state.endTime : params.endTime,
    ...
    this.setState({ echart3: res })
  }
  // 点击店铺排行(金额)时调用商品排行(金额)的接口
  _loadBar31 = async (params = {}) => {
  ...
  _loadTable1 = async (params = {}) => {
    let res = await api.shopCount.tableList1({
      pageSize: '10',
  ...
  _loadTable2 = async (params = {}) => {
    let res = await api.shopCount.tableList2({
      pageSize: '10',
      pageNo: params.current || '1',
      beginTime: params.startTime === undefined ? this.state.startTime : params.startTime,
      endTime: params.endTime === undefined ? this.state.endTime : params.endTime,
  ...
  _loadTable3 = async (params = {}) => {
    let res = await api.shopCount.tableList3({
  ...
  _loadTable4 = async (params = {}) => {
    let res = await api.shopCount.tableList4({
      pageSize: params.pageSize || '20',
      pageNo: params.current || '1',
      beginTime: params.startTime === undefined ? this.state.startTime : params.startTime,
      endTime: params.endTime === undefined ? this.state.endTime : params.endTime,
  ...
  exportData1 = async() => {
    let res = await api.shopCount.downLoad({
  ...
  render() {
    const { rangePicker, tableList1, tableList2, tableList3, tableList4, showAmountMore, showNumberMore, colorLists, echart1, echart2, echart3, table3PageSize = 20, table4PageSize = 20 } = this.state
    // 3个option对应3个图表
    let option1 = bar1Data(echart1, colorLists.colorList1)
    let option2 = bar2Data(echart2, colorLists.colorList2)
    let option3 = bar3Data(echart3)
    return <div>
        ...
        <Row>
          ...
          <Col span={8}>
            <ReactEchartsCore
              echarts={echarts}
              option={option3}
              onChartReady={this.onChartReady3}
              className={style.charts} />
          </Col>
        </Row>
        <div>
